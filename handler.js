...
export async function participantsUpdate({ id, participants, action }) {
 ...
 this.sendFile(id, apii.data, 'pp.jpg', text, null, false, { mentions: [user] }) 
        ...
        case 'daradmin':
        case 'promuovi':
            text = (chat.sPromote || this.spromote || conn.spromote || '@user ```è ora admin```')
        case 'demote':
        case 'quitarpoder':
        case 'retrocedi':
            if (!text)
                text = (chat.sDemote || this.sdemote || conn.sdemote || '@user ```non è piu admin```')
...
export async function groupsUpdate(groupsUpdate) {
    ...
    for (const groupUpdate of groupsUpdate) {
        ...
        if (groupUpdate.subject) text = (chats.sSubject || this.sSubject || conn.sSubject || '```nome modificato in```\n@subject').replace('@subject', groupUpdate.subject)
...
global.dfail = (type, m, conn) => {
    let msg = {
        ...
        owner: 'contaci',
        mods: 'contaci',
        premium: 'comando riservato agli utenti premium',
        group: 'comando riservato ai gruppi',
        private: 'comando riservato alla chat privata',
        admin: 'contaci',
        botAdmin: 'come vuoi che faccia senza admin',
        unreg: 'non sei registrato',
        restrict: 'comando non disponibile'
    }[type]
    let prep = generateWAMessageFromContent(m.chat, { extendedTextMessage: { text: msg, contextInfo: { externalAdReply: { title: 'bot', body: 'bot', sourceUrl: 'bot' }}}}, { quoted: m })
    if (msg) return 
